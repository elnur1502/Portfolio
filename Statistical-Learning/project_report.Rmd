...
c2 <- ggplot(data, aes(x = date)) +
  ...
  ggtitle("Amount of clients by time") +
  settings
...
![Independence graph](Independence_graph.jpg){width=50%}
...
for (feat in c(nominal_feats, ordinal_feats)) {
  data_feat <- data
  data_feat$timesplit <- data_feat$date <= cut_date
  data_feat <- with(data, table(y, data[, feat], dnn = c('timesplit', feat)))
  data_feat <- as.data.frame(data_feat)
  data_feat$NormFreq <- data_feat$Freq / 
...
The boxplots built for "Poutcome" confirms the assumption about changing the policy.
